...
void bypass(Node* root) {
    if (root != nill && root != nullptr) {
        ...
        bypass(root->LeftChild);
        std::cout << root->key.series << ' ' << root->key.number << std::endl;
        bypass(root->RightChild);
    }
}
